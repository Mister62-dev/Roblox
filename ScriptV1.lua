-- Sans Dashboard v2 | Mobile Horizontal
-- Fixed: Icon Draggable, Grab All Tools, GP Door Bypass
...
-- Fire uniquement sur de vrais Remotes
...
IS.Color = Color3.fromRGB(34, 197, 94)
IS.Thickness = 2
...
local function Toggle(parent, icon, label, key, color)
    ...
    Lbl.Text = label; Lbl.TextColor3 = Color3.fromRGB(210,210,210)
...
-- FIXED: teleporte sur le NPC pour trigger les ToolGiver (scripts de proximité)
...
-- ── ICON DRAG — tap = ouvre, glisse = déplace ────────────────────────────────
do
    local dragging  = false
    local dragStart = Vector2.new()
    local posStart  = Icon.Position
    local moved     = false
    local THRESHOLD = 8
    ...
    UIS.InputChanged:Connect(function(inp)
        ...
        Icon.Position = UDim2.new(0,
            math.clamp(posStart.X.Offset + delta.X, 0, vp.X - 54),
            0,
    ...
    while true do
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj.Name == "Money" then
                local p = obj:IsA("BasePart") and obj or obj:FindFirstChildOfClass("BasePart")
                if p then
                    Glide(p.Position + Vector3.new(0,3,0))
                    task.wait(0.2)
                end
            end
        end
...
task.spawn(function() -- Tool Grabber loop (FIXED)
    while true do
        task.wait(S.Delay * 5)
...
task.spawn(function() -- GP Door Bypass (FIXED)
    while true do
        task.wait(S.Delay * 5)
        ...
        for _, obj in ipairs(workspace:GetDescendants()) do
            ...
            if n:find("gp door") or n:find("gpdoor") then
                ...
                if part then
                    HRP.CFrame = CFrame.new(part.Position + part.CFrame.LookVector * 12)
                    Notif("🚪 GP Door passé")
                    task.wait(2)
                end
            end
...
Notif("✅ Dashboard v2 — Bonne game boss man")
