-- Roblox Game Scanner V5 | Landscape | Stop | Copy
...
local MAX = 5000 -- anti-scan infini
...
make("TextLabel",{Size=UDim2.new(0,180,0,18),Position=UDim2.new(0,22,0,4),
    BackgroundTransparency=1,Text="GAME SCANNER  •  V5",
...
make("TextLabel",{Size=UDim2.new(0,220,0,11),Position=UDim2.new(0,22,0,22),
    BackgroundTransparency=1,Text="DELTA READY  •  MAX "..MAX.." INSTANCES",
...
-- BODY  (544 × 246 px)
-- Left: 248px  |  divider: 8px  |  Right: 288px
...
-- TOGGLE (117px × 24px, deux colonnes — gap 14px)
local Toggles={}
...
makeTog("GUI",       "GUI",     0,   42,Left,true)
makeTog("Values",    "Values",  131, 42,Left,true)
makeTog("Meshes",    "Meshes",  0,   70,Left,false)
makeTog("Workspace", "WS",      131, 70,Left,true)
makeTog("RepStorage","RS",      0,   98,Left,true)
makeTog("StarterPk", "SK",      131, 98,Left,true)
secLabel("OPTIONS",124,Left)
makeTog("Console",  "Console",  0,  138,Left,true)
makeTog("Export",   "Export",   131,138,Left,true)

-- LOG (right, 286px large, 168px haut)
...
-- BOUTON SCAN (= STOP pendant le scan)
-- 138px + 10px gap + 138px = 286px
...
-- BOUTON COPY (grisé jusqu'à la fin du scan)
...
ScanBtn.MouseButton1Click:Connect(function()
    ...
    local function scanService(root)
        ...
        while #stack>0 and not stopFlag do
            ...
            counter=counter+1
            if counter>MAX then
                addLog("⚠ Limite "..MAX.." atteinte.",C.Red)
                stopFlag=true break
            end
            if counter%10==0 then task.wait() end
    ...
    setStatus("✔ "..#Res.." éléments — appuie COPIER",C.Accent)
...
addLog("Limite : "..MAX.." instances max.",C.Dim)
