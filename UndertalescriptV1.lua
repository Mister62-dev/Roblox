...
local function Slider(parent, label, minV, maxV, defV, cb)
    ...
    Knob.Position         = UDim2.new(p0, -10, 0.5, -10)
    Knob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
...
-- ── SCAN GP — VERSION CORRIGÉE ────────────────────────────────────────────────
local KEYS = {
    "sans","mini","boss","morph","npc","phase","bendy","last","breath",
    "virus","dernier","souffle","ecran","fatal","mortel","alpha",
    "corruption","poussiere","poussière","roi","multivers","underswap",
    "omega","boule","ts ","swap","fell","dust","error","ink","gaster",
    "chara","asriel","flowey","asgore","undyne","papyrus","mettaton"
}

local function matchKeys(str)
    local s = str:lower()
...
local function ScanGPChars()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj ~= playerChar and not seen[obj] then
            local part = obj:FindFirstChild("HumanoidRootPart")
                      or obj:FindFirstChildOfClass("BasePart")
            if not part then continue end

            local billboard = getBillboardName(obj)
            local combined  = obj.Name .. " " .. (billboard or "")

            if matchKeys(combined) then
                seen[obj] = true
                table.insert(found, {
                    name  = billboard or obj.Name,
                    model = obj,
                    part  = part
                })
            end
    ...
    for _, entry in ipairs(found) do
        ...
        Btn.TextColor3       = Color3.fromRGB(255, 255, 255)
        Btn.TextSize         = 10; Btn.Font = Enum.Font.Gotham
        ...
        if S.HPMonitor then
            ...
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj:IsA("Humanoid") then
                    ...
                    local billboard = obj.Parent and getBillboardName(obj.Parent) or ""
                    if matchKeys(n) or matchKeys(billboard) then
                        HPLbl.Text = "❤️  Boss HP : "
                            .. math.floor(obj.Health) .. " / "
                            .. math.floor(obj.MaxHealth)
                        found = true; break
                    end
                end
            end
